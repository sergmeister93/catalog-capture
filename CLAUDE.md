...
**Phase 7T (Throughput Pass) — 2026-06-12** — three speed/cost levers for per-image extraction (was ~30s/image): (a) `GEMINI_THINKING_BUDGET` (default **0** = thinking disabled) — gemini-2.5-flash's dynamic thinking was previously uncapped, burning latency + output-rate tokens on every call; set `-1` to restore the SDK default, `>0` to cap; (b) `GEMINI_MAX_IMAGE_EDGE_PX` (default 1536) — camera-grade photos are downscaled client-side (Pillow, LANCZOS, JPEG q85) in `real_gemini.prepare_image_bytes` before upload (Gemini tiles images at 768px, so more resolution buys nothing); fails open to original bytes on any decode error; `0` disables; (c) `EXTRACTION_CONCURRENCY` default 3 → 6 (paid-tier RPM headroom is ample). New dep: `pillow>=10`. Tests: `backend/tests/inbound/test_image_downscale.py`. If pricing quality ever looks worse after this, try `GEMINI_THINKING_BUDGET=512` first.
